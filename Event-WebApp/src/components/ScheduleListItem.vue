...
const filteredPerformances = computed((): Performance[] => {
  ...
  return performances.value.filter((perf) => {
    ...
    // Search: performance.description + act.description
    if (filters.searchQuery) {
      const query = filters.searchQuery.toLowerCase();
      const inPerformance = (perf.description || '').toLowerCase().includes(query);
      const inActs = perf.actsIDArr.some((actID) => {
        const act = acts.value.find((a) => a.id === actID);
        return act?.description?.toLowerCase().includes(query);
      });

      if (!inPerformance && !inActs) return false;
    }
