            ...
            <div v-for="stage in stages" :key="stage.id" class="stage">
                ...
                <div class="events" :style="getEventsContainerStyle()">
                    ...
                    <router-link 
                        ...
                        :title="event.name || getActNames(event.actsIDArr) || $t('Unknown')">
                        <strong>
...
const getActNames = (actsArr = []) => {
    return actsArr.map(actID => acts.value.find(act => act.id === actID)?.name || 'Unknown Artist').join(', ');
};
